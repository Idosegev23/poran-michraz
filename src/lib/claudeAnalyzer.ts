...
**קריטי - אסור גרשיים כפולים בתוך ערכים**:
JSON שובר כש-" מופיע בתוך ערך. **בכל מקום שהיית כותב " בעברית — השתמש בגרשיים עבריות ״ (גרשיים, U+05F4)**.
דוגמאות חובה:
- במקום "ש"ח" כתוב "ש״ח"
- במקום "מש"ח" כתוב "מש״ח"
- במקום "כ"א" כתוב "כ״א"
- במקום "בינ"ל" כתוב "בינ״ל"
- במקום "מע"מ" כתוב "מע״מ"
- במקום "שכ"ט" כתוב "שכ״ט"
זה חשוב במיוחד עבור: סכומי כסף, ראשי תיבות עבריים, ציטוטים.`;
...
export async function analyzeTender(documentText: string): Promise<TenderAnalysis> {
  ...
  // Pre-process: convert common Hebrew gershayim patterns from " to ״ (U+05F4)
  // These break JSON when they appear inside string values
  const HEBREW_ABBREV = [
    'ש"ח', 'מש"ח', 'מע"מ', 'כ"א', 'בינ"ל', 'שכ"ט', 'דו"ח',
    'תכ"מ', 'תק"ל', 'יח"ל', 'תפ"א', 'בנק'
  ];
  let preprocessed = 0;
  for (const abbrev of HEBREW_ABBREV) {
    const fixed = abbrev.replace('"', '״');
    const before = jsonStr;
    jsonStr = jsonStr.split(abbrev).join(fixed);
    if (before !== jsonStr) preprocessed++;
  }
  // Also fix any remaining Hebrew letter + " + Hebrew letter pattern (catch-all)
  const hebrewQuotePattern = /([א-ת])"([א-ת])/g;
  const beforeRegex = jsonStr;
  jsonStr = jsonStr.replace(hebrewQuotePattern, '$1״$2');
  if (beforeRegex !== jsonStr) preprocessed++;
  if (preprocessed > 0) {
    console.log(`[ANALYZE] Pre-processed ${preprocessed} Hebrew gershayim pattern(s)`);
  }
